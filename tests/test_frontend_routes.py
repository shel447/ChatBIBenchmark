...
class FrontendRouteTests(unittest.TestCase):
    ...
    def test_frontend_index_served(self):
        ...
        self.assertIn("导出结果报告", response.text)
        self.assertIn("报告格式", response.text)
        self.assertIn("执行趋势分析", response.text)
        self.assertIn("用例集准确率趋势", response.text)
        self.assertIn("用例准确率趋势", response.text)
        self.assertIn("回归劣化预警", response.text)
