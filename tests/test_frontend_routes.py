...
class FrontendRouteTests(unittest.TestCase):
    ...
    def test_frontend_index_served(self):
        ...
        self.assertIn("执行准确率", response.text)
        self.assertIn("测试套件准确率", response.text)
        self.assertIn("图表类型准确率", response.text)
        self.assertIn("任务成功率", response.text)
        self.assertIn("事实精度", response.text)
        self.assertIn("已接入执行", response.text)
